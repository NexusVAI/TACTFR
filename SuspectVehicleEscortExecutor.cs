...
namespace EF.PoliceMod.Executors
{
    ...
    public class SuspectVehicleEscortExecutor
    {
        ...
        internal static class VehicleEscortLine
        {
            ...
            internal static bool ShouldAutoDoors(ArrestActionStyle style) => true;
        ...
        private void OnVehicleInteract(EscortVehicleInteractEvent e)
        {
            ...
            // PullOver 旁路：在 None/Restrained 阶段按 E，直接进入押送并允许继续本次上车流程。
            if (pullOverBypass && (_stateHub.Is(SuspectState.None) || _stateHub.Is(SuspectState.Restrained)))
            {
                try
                {
            ...
            {
                if (_requireFollowBeforeBoard && !pullOverBypass)
                {
                ...
                try
                {
                    if (GetStyle() == ArrestActionStyle.CuffAndLead && !player.IsInVehicle())
                    {
                        ...
                        try
                        {
                            bool nearSuspect = IsPlayerNearSuspectInteractionPoint(suspect, player, 2.0f);
                            bool nearVehicleContext = IsVehicleBoardingContextNear(suspect, player, nearVeh, pullOverBypass ? 18.0f : 8.0f);
                            if (!nearSuspect && !nearVehicleContext)
                            {
                                NotifyVehicleInteractReject("~y~离嫌疑人/车辆太远，请贴近车门再按 E");
                                return;
                            }
                        }
                        catch { }
                ...
                {
                    bool nearSuspect = IsPlayerNearSuspectInteractionPoint(suspect, player, 2.0f);
                    bool nearVehicleContext = IsVehicleBoardingContextNear(suspect, player, vehicle, pullOverBypass ? 20.0f : 10.0f);
                    if (!nearSuspect && !nearVehicleContext)
                    {
                        ModLog.Info("[Escort][Vehicle] E pressed but suspect/vehicle context too far");
                        NotifyVehicleInteractReject("~y~请让嫌疑人靠近车辆后再按 E");
                        return;
                    }
                }
            ...
            // 其他状态不处理
            ModLog.Info("[Escort][Vehicle] E pressed but no action for current suspect state (need Escorting/InVehicle)");
